...
const MAIN_SESSION_STRING = process.env.TELEGRAM_SESSION_STRING || "";
const GLOBAL_CHANNEL_ID = process.env.TELEGRAM_CHANNEL_ID || "-1004460769144";
...
async function generateVariants(record, uploadClient) {
    ...
    try {
        ...
        const messageId = typeof record.messageId === 'object' ? (record.messageId.high || record.messageId.original || record.messageId.mid || Object.values(record.messageId)[0]) : parseInt(record.messageId);
...
app.post('/api/upload', upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        const recordCacheDir = path.join(CACHE_DIR, newRecordId);
        
        if (isImage) {
            if (!fs.existsSync(recordCacheDir)) fs.mkdirSync(recordCacheDir, { recursive: true });
            
            const qualities = {
                low: { width: 400, quality: 50 },
                mid: { width: 1024, quality: 78 },
                high: { width: 2000, quality: 85 }
            };
            
            for (const [qName, config] of Object.entries(qualities)) {
                const outPath = path.join(recordCacheDir, `${qName}.jpg`);
                try {
                    ...
                    const result = await uploadClient.sendFile(targetChannel, {
                        file: outPath,
                        caption: `${newRecordId} | ${qName} | ${file.originalname}`,
                        forceDocument: true,
                    });
                    messageIds[qName] = result.id;
                    
                    // Pre-warm memory cache
                    const diskBuf = fs.readFileSync(outPath);
                    memoryCacheSet(`${newRecordId}:${qName}`, diskBuf);
                    
                } catch (err) {
                    console.error(`Error processing ${qName}:`, err);
                }
            }
        ...
        const defaultId = typeof messageIds === 'object' ? (messageIds.high || messageIds.low) : messageIds;
        const telegramLink = `https://t.me/c/${cleanChannelId}/${defaultId}`;
...
// ==========================================
// EFFICIENT MEDIA STREAMING (HTTP Range Support)
// ==========================================
async function streamMediaFromTelegram(req, res, record, uploadClient, messageId) {
    try {
        const messages = await uploadClient.getMessages(record.channelId || GLOBAL_CHANNEL_ID, { ids: [messageId] });
        if (!messages || messages.length === 0 || !messages[0].media) {
        ...
        if (range && fileSize > 0) {
            ...
            try {
                ...
                console.error("iterDownload streaming error:", err.message);
            ...
            try {
                ...
                console.error("iterDownload full file error:", err.message);
                if (!res.writableEnded) res.end();
                return;
            }
            res.end();
        ...
        console.error("streamMediaFromTelegram error:", err);
...
app.get('/api/image/:recordId', async (req, res) => {
    ...
    // Determine requested quality (default: low, ?q=high for full quality)
    const quality = ['thumb', 'low', 'mid', 'high'].includes(q) ? q : 'low';
    ...
    try {
        ...
        const targetChannel = record.channelId || GLOBAL_CHANNEL_ID;
        let messageId = parseInt(record.messageId);
        if (typeof record.messageId === 'object') {
            // Fetch exact requested quality if available, otherwise fallback
            messageId = record.messageId[quality] || record.messageId.high || record.messageId.original || Object.values(record.messageId)[0];
        }

        // Stream directly from Telegram with HTTP Range support
        await streamMediaFromTelegram(req, res, record, uploadClient, messageId);
